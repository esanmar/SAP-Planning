...
function CountDownObject() {
	...
	this.NUM_OF_ELEMENTS = 6; // number of flip-elements(from 1 to 9)
	this.NUM_OF_REMOVE_ELEMENTS = 2; // number of elements which will be removed from right side countdown (2 - remove seconds, 4 - remove minutes, ...)
    this.TIME_ANIMATION = 900; // time of flip animation in milliseconds(from 50 to 950)
	this.BACK_COLOR = "#00a19b"; // flip-element back color
	//edsa01 this.DIGITS_COLOR = "#f0d070"; // digits color on flip-elements
	...
	function TimerEnd(){
		...
		CanvasContext.font = 30*param.elementSize + "px Open Sans";
		CanvasContext.textAlign = "center";
		CanvasContext.fillText("PROCESO NO CERRADO", Canvas.width/2, 84.5*param.elementSize);
	}
	...
	function DrawText(){
		...
		if(num == 1){
            ...
            CanvasContext.fillText("SECONDS", param.sec1Position - 80*0.05*param.elementSize - offsetText, 150*param.elementSize);
		}
        if(num == 3){
			CanvasContext.fillText("MINUTOS", param.min1Position + 80*0.5*param.elementSize, 150*param.elementSize);
		}else if(num > 3){
			...
			CanvasContext.fillText("MINUTOS", param.min1Position - 80*0.05*param.elementSize - offsetText, 150*param.elementSize);
		}
		if(num == 5){
			CanvasContext.fillText("HORAS", param.hour1Position + 80*0.5*param.elementSize, 150*param.elementSize);
		}else if(num > 5){
            ...
            CanvasContext.fillText("HORAS", param.hour1Position - 80*0.05*param.elementSize - offsetText, 150*param.elementSize);
		}
        ...
        if(num == 7){
            ...
            CanvasContext.fillText("DAYS", param.day1Position - 80*0.05*param.elementSize - offsetText, 150*param.elementSize);
		}else{
			CanvasContext.fillText("DAYS", param.day1Position - 80*0.6*param.elementSize - offsetText, 150*param.elementSize);
		}
	}
	...
	function Main(){
		...
		if( timeAndDateNow.hours >= 1 ) {
	     backC = "#00a19b";
			 digitsC = "#d6dfe6";
			 this.BACK_COLOR = "#00a19b"; // Verde #437921   
	     //edsa01 this.DIGITS_COLOR = "#d6dfe6"; 
	     this.DIGITS_COLOR = "#ffffff"; 
		} 
		else {
			if( timeAndDateNow.mins <= 15) {
				 this.BACK_COLOR = "#B00D36"; // Rojo  #e33c4f
		     this.DIGITS_COLOR = "#d6dfe6"; 
		     backC = "#B00D36";
				 //edsa01 digitsC = "#d6dfe6";
				 digitsC = "#ffffff";
				} else {
					  this.DIGITS_COLOR = "#b2822f"; // Naranja #f07000
						this.TEXT_COLOR = "#d6dfe6"; 
						backC = "#b2822f";
				 	  //edsa01 digitsC = "#d6dfe6";
				 	  digitsC = "#ffffff";
				}
